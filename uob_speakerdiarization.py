...
def load_speaker_vector():
    # TODO: save pretrained model in local???
...
#                                   Malaya-Speech                                     #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
def load_vad(y, sr, frame_duration_ms=30, threshold_to_stop=0.3):
    vad = malaya_speech.vad.deep_model(model = 'vggvox-v2')   # ? HOW ABOUT OTHER VAD MODELS?
...
def spectral_clustering(speaker_vector, grouped_vad, min_clusters=2, max_clusters=5):
    ...
    result_diarization_sc_model[:5]
       
    return result_diarization_sc_model


## Static N speakers using sklearn clustering
def n_speakers_clustering(speaker_vector, grouped_vad, n_speakers=2, model=['kmeans','spectralcluster']):
    ...
    if model == 'kmeans': 
        ...
        result_diarization_kmeans_model[:5]
        ...
        result_diarization_spectralclustering_model = malaya_speech.diarization.n_clustering(grouped_vad, speaker_vector,
                                                                                ...
                                                                                norm_function = lambda x: x)
        result_diarization_spectralclustering_model[:5]
...
def speaker_change_detection( grouped_vad, y, sr, 
    ...
    result_diarization_sc_splitted_model = malaya_speech.diarization.spectral_cluster(splitted_speakervec, 
                                                                                       ...
                                                                                       min_clusters = min_clusters,
                                                                                       max_clusters = max_clusters)
    result_diarization_sc_splitted_model[:5]
