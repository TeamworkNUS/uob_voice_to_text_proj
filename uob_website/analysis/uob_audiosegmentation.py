...
def prepare_for_analysis(sound, 
                         silence_thresh=-70, 
                         min_silence_len = 700, 
...
def audio_segmentation(name,path,savetime):
    ## loading
    ...
    joint_length_limit = 60*1000
    withjoint = True
    
    ## output
    # total_chunks, nowtime, chunks_folder = prepare_for_analysis(name, sound, savetime, silence_thresh, min_silence_len, length_limit, abandon_chunk_len, joint_silence_len, joint_length_limit, withjoint)
    chunks = prepare_for_analysis(sound, silence_thresh, min_silence_len, length_limit, abandon_chunk_len, joint_silence_len, joint_length_limit, withjoint)
    ...
    chunks_folder = os.path.join(path, 'chunks_'+namef+'_'+savetime)
...
withjoint: {withjoint} \n 
    ...
    return total_chunks, chunks_folder
